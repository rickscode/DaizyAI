...
class Login extends Component {
	...
	return (
		<>
			...
			<NavLink to="/login" className={`flex-1 justify-center transition py-4 px-4 pr-8 rounded-t-md flex text-${this.props.location.pathname === "/login" ? "yellow-750" : "gray-600"} font-medium bg-${this.props.location.pathname === "/login" ? "white" : "gray-300"} hover:bg-${this.props.location.pathname === "/login" ? "white" : "gray-100"} cursor-pointer`}>
  <Circle color={this.props.location.pathname === "/login" ? "yellow-500" : "gray-400"}>
    <CheckIcon className="h-4 w-4 text-black" aria-hidden="true" />
  </Circle>
  Login
</NavLink>
			  ...
			  <Switch>
				...
				<a href={`https://www.daizyai.tech/`} className="block text-center bg-gray-100 text-gray-500 text-sm p-3 rounded-b-lg hover:bg-gray-200 cursor-pointer">
...
<form onSubmit={onSignup}>
					...
					<div className="flex flex-col">
						...
						<button
  						type="submit"
  						className="hover:bg-yellow-750 bg-yellow-500 font-medium rounded-lg text-lg px-4 py-2 bg-gray-200 text-white mt-4 border border-gray-300 inline-block">
  						Sign Up
						</button>
					</div>
                    </div>
